...
// ============= Your Code Here =============
type FindAllHelper<F extends string, U extends string, C extends number[] = [], K extends any[] = []> =
  F extends `${string}${infer R}`
  ? F extends `${U}${string}`
    ? FindAllHelper<R, U, [...C, K['length']], [...K, 0]>
    : FindAllHelper<R, U, C, [...K, 0]>
  : C;
