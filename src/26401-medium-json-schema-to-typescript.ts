...
type JSONSchema2TS<T> = T extends { type: infer P }
  ? P extends keyof Primitive
...
type HandleObject<T> = T extends { properties: infer P }
  ? T extends { required: infer R extends unknown[] }
    ? Omit<
        {
          [K in keyof P] ?: JSONSchema2TS<P[K]>;
        } & {
          [K in keyof P as K extends R[number] ? K : never]: JSONSchema2TS<
            P[K]
          >;
        },
        never
