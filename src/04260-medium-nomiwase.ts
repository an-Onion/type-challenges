...
// ============= Your Code Here =============
type String2Union<S extends string> = S extends `${infer L}${infer R}` ? L | String2Union<R> : never;

type AllUnionCombinations<U extends string> = [U] extends [never]
  ? ''
  : '' | {[K in U]: `${K}${AllUnionCombinations<Exclude<U, K>>}`}[U]

type AllCombinations<S extends string> = AllUnionCombinations<String2Union<S>>;
